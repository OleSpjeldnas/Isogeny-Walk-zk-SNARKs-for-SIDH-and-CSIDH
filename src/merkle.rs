use super::F;
use ark_crypto_primitives::{crh::poseidon, merkle_tree::{Config, IdentityDigestConverter}, MerkleTree};
...
pub fn poseidon_parameters() -> PoseidonConfig<F> {
    ...
    //  [0, 1, 1]]
    let mds = vec![
        vec![F::from(0), F::from(0), F::from(1)],
        vec![F::from(1), F::from(1), F::from(0)],
        vec![F::from(0), F::from(1), F::from(1)],
    ];
