...
fn main() {
    ...
    let witness: DensePolynomial<F> = DensePolynomial {
        coeffs: lines_from_file_2("new_coeffs.txt").unwrap(),
    };
    // Witness(x+1)
    ...
    // psi
    let psi: DensePolynomial<F> = DensePolynomial {
        coeffs: lines_from_file_2("psi_coeffs.txt").unwrap(),
    };
    ...
    if b {
        ...
        println!(
            "Proof Size: {} kB",
            ((size1 + size2 + size3 + size4 + size5 + size6 + size7) as f32) / 1024f32
        );
