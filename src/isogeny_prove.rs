...
pub fn prove(
    ...
    l_list: Vec<usize>, rep_param: usize,
    ...
    let b_witness: DensePolynomial<F> = witness + blinding_factor;
    ...
    let witness_evals: Vec<F> = D_0.clone().into_par_iter().map(|x| b_witness.evaluate(&x)).collect();
    let psi_evals: Vec<F> = D_0.clone().into_par_iter().map(|x| psi.evaluate(&x)).collect();
    ...
    witness_evals_merkle =
        vec![witness_evals_merkle, vec![vec![Fp::from(0), Fp::from(0)]; 2u64.pow(k) as usize - s_ord as usize]]
            .concat();
    psi_evals_merkle =
        vec![psi_evals_merkle, vec![vec![Fp::from(0), Fp::from(0)]; 2u64.pow(k) as usize - s_ord as usize]].concat();
    ...
    let witness_mtree: FieldMT = FieldMT::new(&leaf_crh_params, &two_to_one_params, w_merkle_slice).unwrap();

    // Merkle tree of psi evaluations on E
    let psi_mtree: FieldMT = FieldMT::new(&leaf_crh_params, &two_to_one_params, psi_merkle_slice).unwrap();
    ...
    let c_evals: Vec<F> = D_0.into_par_iter().map(|x| c.evaluate(&x)).collect();
    let mut c_evals_merkle: Vec<Vec<Fp>> = c_evals.par_iter().map(|x| vec![x.c0, x.c1]).collect();

    c_evals_merkle =
        vec![c_evals_merkle, vec![vec![Fp::from(0), Fp::from(0)]; 2u64.pow(k) as usize - s_ord as usize]].concat();
    let c_merkle_slice: Vec<&[Fp]> = c_evals_merkle.par_iter().map(|x| x.as_slice()).collect();

    // Merkle tree of witness evaluations on E
    let c_mtree: FieldMT = FieldMT::new(&leaf_crh_params, &two_to_one_params, c_merkle_slice).unwrap();
    ...
    let p: DensePolynomial<F> =
        DensePolynomial { coeffs: vec![vec![F::from(0); E - n], vec![F::new(zeta_vec[0], Fp::from(0))]].concat() }
            ...
            + DensePolynomial {
                coeffs: vec![vec![F::from(0); E - n], vec![F::new(zeta_vec[1], Fp::from(0))]].concat(),
            }
            .naive_mul(&(b_witness.clone() + DensePolynomial { coeffs: vec![-challenge_vals[1]] }))
            .div(&DensePolynomial { coeffs: vec![-gz, F::from(1)] })
            + DensePolynomial {
                coeffs: vec![vec![F::from(0); E - n], vec![F::new(zeta_vec[2], Fp::from(0))]].concat(),
            }
            .naive_mul(&(b_witness + DensePolynomial { coeffs: vec![-challenge_vals[2]] }))
            .div(&DensePolynomial { coeffs: vec![-ggz, F::from(1)] })
            + DensePolynomial {
                coeffs: vec![vec![F::from(0); E - n - 1], vec![F::new(zeta_vec[3], Fp::from(0))]].concat(),
            }
...
pub fn verify(
    ...
    n: &u64, s_ord: u64, y_start: &F, y_end: &F, l_list: Vec<usize>, rep_param: usize,
) -> bool {
    ...
    for (i, index) in indices_first.iter().enumerate() {
        ...
        let asserted_p: F = F::new(zeta_vec[0], Fp::from(0)) * x_0.pow([E - n]) * (witness_val - challenges[0])
            / (x_0 - z)
        ...
        assert!(additional_paths[0][i]
            .verify(&leaf_crh_params, &two_to_one_params, &roots[0], [witness_val.c0, witness_val.c1])
            .unwrap());

        assert!(additional_paths[1][i]
            .verify(&leaf_crh_params, &two_to_one_params, &roots[1], [psi_val.c0, psi_val.c1])
            .unwrap());
        assert!(additional_paths[2][i]
            .verify(&leaf_crh_params, &two_to_one_params, &roots[2], [c_val.c0, c_val.c1])
            .unwrap());
    }
    true
}
