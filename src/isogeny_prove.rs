...
pub fn prove(
    ...
    let D_0: Vec<F> = (0..s_ord).into_par_iter().map(|i| r * s.pow([i])).collect();
    let witness_evals: Vec<F> = D_0
        .clone()
        .into_par_iter()
        .map(|x| b_witness.evaluate(&x))
        .collect();
    let psi_evals: Vec<F> = D_0
        .clone()
        .into_par_iter()
        .map(|x| psi.evaluate(&x))
        .collect();

    let mut witness_evals_merkle: Vec<Vec<Fp>> =
        witness_evals.par_iter().map(|x| vec![x.c0, x.c1]).collect();
    let mut psi_evals_merkle: Vec<Vec<Fp>> =
        psi_evals.par_iter().map(|x| vec![x.c0, x.c1]).collect();
    let k: u32 = (((s_ord as f32).log2()).ceil()) as u32;
    witness_evals_merkle = vec![
        witness_evals_merkle,
        vec![vec![Fp::from(0), Fp::from(0)]; 2u64.pow(k) as usize - s_ord as usize],
    ]
    .concat();
    psi_evals_merkle = vec![
        psi_evals_merkle,
        vec![vec![Fp::from(0), Fp::from(0)]; 2u64.pow(k) as usize - s_ord as usize],
    ]
    .concat();
    ...
    let alpha_2: F = F::new(
        poseidon::CRH::<Fp>::evaluate(&params, vec![alpha_1.c0]).unwrap(),
        Fp::from(0),
    );
    let alpha_3: F = F::new(
        poseidon::CRH::<Fp>::evaluate(&params, vec![alpha_2.c0]).unwrap(),
        Fp::from(0),
    );
    let alpha_4: F = F::new(
    ...
    let c: DensePolynomial<F> = compute_c(
        c1,
        c2,
        c3,
        c4,
        &vec![alpha_1, alpha_2, alpha_3, alpha_4],
        &n,
    );

    // Evaluate C(x) on E and commit
    let c_evals: Vec<F> = D_0
        .clone()
        .into_par_iter()
        .map(|x| c.evaluate(&x))
        .collect();
    let mut c_evals_merkle: Vec<Vec<Fp>> = c_evals.par_iter().map(|x| vec![x.c0, x.c1]).collect();
    ...
    // Merkle tree of witness evaluations on E
    let c_mtree: FieldMT =
        FieldMT::new(&leaf_crh_params, &two_to_one_params, c_merkle_slice).unwrap();
    ...
    let z: F = F::new(
        poseidon::CRH::<Fp>::evaluate(&params, roots.clone()).unwrap(),
        Fp::from(0),
    );
    ...
    let p: DensePolynomial<F> = DensePolynomial {
        coeffs: vec![
            vec![F::from(0); E - n],
            vec![F::new(zeta_vec[0], Fp::from(0))],
        ]
        .concat(),
    }
    .naive_mul(
        &(b_witness.clone()
            + DensePolynomial {
                coeffs: vec![-challenge_vals[0]],
            }),
    )
    .div(&DensePolynomial {
        coeffs: vec![-z, F::from(1)],
    }) + DensePolynomial {
        coeffs: vec![
            vec![F::from(0); E - n],
            vec![F::new(zeta_vec[1], Fp::from(0))],
        ]
        .concat(),
    }
    .naive_mul(
        &(b_witness.clone()
            + DensePolynomial {
                coeffs: vec![-challenge_vals[1]],
            }),
    )
    .div(&DensePolynomial {
        coeffs: vec![-gz, F::from(1)],
    }) + DensePolynomial {
        coeffs: vec![
            vec![F::from(0); E - n],
            vec![F::new(zeta_vec[2], Fp::from(0))],
        ]
        .concat(),
    }
    .naive_mul(
        &(b_witness.clone()
            + DensePolynomial {
                coeffs: vec![-challenge_vals[2]],
            }),
    )
    .div(&DensePolynomial {
        coeffs: vec![-ggz, F::from(1)],
    }) + DensePolynomial {
        coeffs: vec![
            vec![F::from(0); E - n - 1],
            vec![F::new(zeta_vec[3], Fp::from(0))],
        ]
        .concat(),
    }
    .naive_mul(
        &(psi
            + DensePolynomial {
                coeffs: vec![-challenge_vals[3]],
            }),
    )
    .div(&DensePolynomial {
        coeffs: vec![-z, F::from(1)],
    }) + DensePolynomial {
        coeffs: vec![F::new(zeta_vec[4], Fp::from(0))],
    }
    .naive_mul(
        &(c + DensePolynomial {
            coeffs: vec![-challenge_vals[4]],
        }),
    )
    .div(&DensePolynomial {
        coeffs: vec![-z, F::from(1)],
    ...
    let (paths_fri, points_fri, roots_fri, indices) =
        fri_prove(p.clone(), l_list, s, r, s_ord, rep_param);
...
pub fn verify(
    ...
    roots_fri: Vec<Fp>,
    roots: Vec<Fp>,
    paths_fri: Vec<FieldPath>,
    additional_paths: Vec<Vec<FieldPath>>,
    points_fri: Vec<F>,
    additional_points: Vec<Vec<F>>,
    g: F,
    s: F,
    r: F,
    n: &u64,
    ...
    for (i, index) in indices_first.iter().enumerate() {
        ...
        let asserted_p: F = F::new(zeta_vec[0], Fp::from(0))
            * x_0.pow(&[E - n])
            * (witness_val - challenges[0])
            / (x_0 - z)
            + F::new(zeta_vec[1], Fp::from(0)) * x_0.pow(&[E - n]) * (witness_val - challenges[1])
                / (x_0 - gz)
            + F::new(zeta_vec[2], Fp::from(0)) * x_0.pow(&[E - n]) * (witness_val - challenges[2])
                / (x_0 - ggz)
            + F::new(zeta_vec[3], Fp::from(0)) * x_0.pow(&[E - n - 1]) * (psi_val - challenges[3])
                / (x_0 - z)
            + F::new(zeta_vec[4], Fp::from(0)) * (c_val - challenges[4]) / (x_0 - z);
        assert_eq!(asserted_p, points_first[i]);
        // Verify Merkle Paths
        assert!(additional_paths[0][i]
            .verify(
                &leaf_crh_params,
                &two_to_one_params,
                &roots[0],
...
pub fn final_poly(y_end: &F, p: DensePolynomial<F>, g: F, T: u64) -> DensePolynomial<F> {
    ...
    .div(&DensePolynomial {
        coeffs: vec![-g.pow(&[T - 1]), F::from(1)],
    })
}

//Returns the composite polynomial
pub fn compute_c(
    c1: DensePolynomial<F>,
    c2: DensePolynomial<F>,
    c3: DensePolynomial<F>,
    c4: DensePolynomial<F>,
    alphas: &Vec<F>,
    T: &usize,
) -> DensePolynomial<F> {
    let E: usize = 32 * T / 9;
    ...
    c1.naive_mul(&DensePolynomial {
        coeffs: vec![vec![F::from(0); deg_1], vec![alphas[0]]].concat(),
    }) + c2.naive_mul(&DensePolynomial {
        coeffs: vec![vec![F::from(0); deg_3], vec![alphas[1]]].concat(),
    }) + c3.naive_mul(&DensePolynomial {
        coeffs: vec![vec![F::from(0); deg_2], vec![alphas[2]]].concat(),
    }) + c4.naive_mul(&DensePolynomial {
...
pub fn mod_poly_poly(
    ...
    let temp: DensePolynomial<F> = p_cubed
        ...
        + DensePolynomial {
            coeffs: vec![F::from(1488u128)],
        }
        .naive_mul(&(p_squared_q + q_squared_p))
        + DensePolynomial {
            coeffs: vec![-F::from(162000u128)],
        }
        .naive_mul(&(p_squared + q_squared))
        + DensePolynomial {
            coeffs: vec![F::from(40773375u128)],
        }
        .naive_mul(&pq)
        + DensePolynomial {
            coeffs: vec![F::from(8748000000u128)],
        }
        .naive_mul(&(p + q))
        + DensePolynomial {
            coeffs: vec![-F::from(157464000000000u128)],
        };

    temp.naive_mul(&DensePolynomial {
        coeffs: vec![-g.pow(&[T as u64 - 1]), F::from(1)],
    })
    .div(&DensePolynomial {
        coeffs: [vec![-F::from(1)], vec![F::from(0); T - 1], vec![F::from(1)]].concat(),
    })
}
...
pub fn psi_poly(
    ...
    let x_1_poly: DensePolynomial<F> = DensePolynomial {
        coeffs: vec![-g_pow, F::from(1)],
    }
    .naive_mul(&DensePolynomial {
        coeffs: vec![-g_pow * g, F::from(1)],
    });
...
pub fn psi_challenge(
    y_witness: &F,
    y_witness_plusplus: &F,
    y_psi: &F,
    x_0: &F,
    n: &u64,
    g: &F,
    ...
    g_prefactor * ((y_witness - y_witness_plusplus) * y_psi - F::from(1))
        / (x_0.pow(&[*n]) - F::from(1))
}
